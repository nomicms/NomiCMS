...
<link rel="stylesheet" href="/design/styles/<? echo User::settings('theme')?>/style.css" />
<link href="https://fonts.googleapis.com/css?family=Open+Sans:300,600" rel="stylesheet">
<link rel="shortcut icon" href="/design/styles/<? echo User::settings('theme')?>/favicon.ico" type="image/x-icon" />
</head>
<body>
...
<?
	...
	{
		echo '<div class="main">Вы уверенны что хотите удалить "'.$name.'"?</div><div class="menu bm flex"><a href="?'.$link.'">'.img('delete.png').' Да, удалить!</a> <a style="cursor: pointer" onclick="history.go(-1)">'.img('link.png').' Отменить</a></div>';
	}
	...
	public function footer()
	{
		...
		if (file_exists(R.'/design/styles/'.$theme.'/footer.php')) {
			...
			self::ads(0);
			self::div('footer', '<a href="/online">'. Language::config('online').': '.$o. ' </a> | '.$g.'<br>Gen: '.round(microtime(true) - $gens, 4));
		}
		echo '</body></html>';
	}
}
